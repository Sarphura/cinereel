...
│                                                                          │
│  ┌─────────────────────────┐         ┌──────────────────────────────┐    │
│  │ Application Server      │         │ Hyper Sidecar                │    │
│  │ (apps/service, .NET 10) │  HTTP   │ (apps/sidecar, Node 22)      │    │
...
- **Application Server** — .NET 10 process hosted at `apps/service` (revived from the legacy skeleton per ADR 0011). Owns the application domain: subscription registry, metadata cache, poster wall, Jellyfin bridge, MonoTorrent scheduler, torrent packaging, auth, RBAC. Talks to the Hyper Sidecar over loopback HTTP with a shared-secret token (NSwag-generated client). Hosts MonoTorrent as an embedded client. Default port `127.0.0.1:8090`. Also serves the SPA from `apps/web/dist/` at the root.
- **Hyper Sidecar** — Node 22 + NestJS process (`apps/sidecar`) that wraps `hyper-sdk`. Owns no business logic. Drive mount/unmount, swarm announce/join, file read/write over Hyperdrive. Default port `127.0.0.1:4201`. Spawned by the Application Server as a child process (ADR 0055). Linked lifecycle — if it dies, the Application Server exits (ADR 0017, 0056).
...
├── drive-index.json               # UUID → drive metadata (Sidecar)
├── sidecar.token                  # 32-byte hex shared secret (Sidecar ↔ App Server)
...
- App Server spawns Sidecar as a child process; both die together (ADR 0017, 0055).
- App Server verifies Sidecar `X-Cinereel-Version` matches before proceeding (ADR 0033).
- Sidecar bound to `127.0.0.1`; App Server also bound to `127.0.0.1` (ADR 0010, 0026).
...
**Hyper Sidecar**:
The Node + NestJS process (`apps/sidecar`) that wraps `hyper-sdk` and exposes drive and swarm operations over HTTP. Owns no business logic.
_Avoid_: backend, API, service

**Application Server**:
The .NET 10 process hosted at `apps/service` (revived from the legacy skeleton per ADR 0011) that owns the application domain — subscription registry, metadata cache, poster wall, search, Jellyfin bridge, MonoTorrent scheduler, and torrent packaging. Talks to the Hyper Sidecar over loopback HTTP with a shared-secret token. Talks to Jellyfin over its own API. Hosts MonoTorrent as an embedded client.
_Avoid_: orchestrator, frontend backend, NestJS service, legacy service

**Corestore**:
The Sidecar's on-disk SQLite-backed store for all Hyperdrive and Bee cores. Located at `<CINEREEL_DATA_DIR>/corestore/` (ADR 0046). One per node.
_Avoid_: db, store, database

**Sidecar Token**:
A 256-bit random hex string at `<CINEREEL_DATA_DIR>/sidecar.token` shared between Sidecar and Application Server for HTTP authentication (ADR 0010, 0049). Mode 0600. Auto-generated on first startup.
...
**Drive Index**:
The node-local persisted registry in the Hyper Sidecar (`<CINEREEL_DATA_DIR>/drive-index.json`) mapping drive UUID → `{ name, type, createdAt }`. Source of truth for "what resource drives do I have mounted". Loaded once at Sidecar startup (ADR 0045, 0048).
_Avoid_: drive list, registry, store

**Mount**:
The act of opening a Hyperdrive (local or remote) and registering it in the in-memory `DriveRegistry`. Mounted drives are reachable through the Hyper Sidecar's HTTP file/tree APIs. Local mounts survive Sidecar restart via the Drive Index; remote mounts are re-mounted by the Application Server (ADR 0050).
_Avoid_: attach, bind
...
**DHT Announce**:
The Sidecar's startup action of announcing each mounted drive's discovery key to the Hyperswarm DHT so peers can find this node. Best-effort; failure does not block startup (ADR 0048).
...
_Avoid_: sidecar, info file, metadata XML
...
**Trailer**:
An MP4 file at `/trailer.mp4` next to a Media Item in a publisher's resource drive. Streamed by the SPA via HTTP Range through the Sidecar. Cached locally by the Application Server with a 1 GB LRU cap (ADR 0054). Not auto-fetched from external APIs (ADR 0057).
...
**ProblemDetails**:
The RFC 9457 standard error response shape used by both the Sidecar and the Application Server. Stable `type` URIs (e.g. `https://cinereel.dev/errors/drive-not-mounted`) are switchable from client code (ADR 0032, 0051).
_Avoid_: error response, error body

## Architecture decisions

The Cinereel architecture is fixed by 63 ADRs in `docs/adr/`. Each ADR captures one decision in context. Newcomers should skim them in batches:
...
- **Sidecar architecture (mirrors C# shape, adapter layers, range controller)** (0044–0053)
- **Operational details (spawn, cache, CI, config, bootstrap, first-time UX)** (0054–0063)
...
- The Sidecar and Application Server are tightly version-coupled (ADR 0033). Upgrading one requires upgrading the other.
...
- The Sidecar is the only place that imports `hyper-sdk` directly. Application Server code accesses drives via `ISidecarClient` (NSwag-generated). This boundary is enforced by `scripts/check-sdk-boundary.sh`.
