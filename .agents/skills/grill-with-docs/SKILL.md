...
description: 通过不间断的追问来打磨某个方案或设计，并在过程中持续产出文档（ADR 与术语表）。
...
- 生成或修改的所有文档（`CONTEXT.md`、ADR、术语表、笔记、报告、Markdown 等）使用简体中文。
...
- 领域中一旦确立的规范术语（如 `Order`、`Invoice`、`Customer` 等 `CONTEXT.md` 中的术语），原样复用，不得翻译或意译。

中文化时只翻译自然语言说明、标题、模板文字、UI 文案（`description`、`display_name`、`short_description` 等元数据）。示例定义与示例代码中的规范领域术语保留原文。

---

发起一次 `/grilling` 会话，并启用 `/domain-modeling` Skill。