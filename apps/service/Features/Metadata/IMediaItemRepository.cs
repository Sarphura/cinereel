...
public sealed class InMemoryMediaItemRepository : IMediaItemRepository
{
    ...
    public Task<MediaItemEntity?> FindByIdAsync(MediaItemId id, CancellationToken cancellationToken = default) => Task.FromResult(_items.GetValueOrDefault(id.Value));
    public Task<IReadOnlyList<MediaItemEntity>> ListBySubscriptionAsync(SubscriptionId subscriptionId, CancellationToken cancellationToken = default) =>
        Task.FromResult<IReadOnlyList<MediaItemEntity>>(_items.Values.Where(item => item.SubscriptionId == subscriptionId.Value).ToList());
