/**
 * JSON Schemas (Fastify 5 strict validation).
 *
 ...
 *
 * Lives in `controllers/schemas.ts` because the schemas are tightly
 * coupled to the HTTP routes that consume them; controllers stay thin
 * (just `register(app)`) and the schema bodies live one hop away.
 */
...
export const AnnounceBody = {
  ...
  // entirely (curl without -d, Apifox without payload, idempotent
  // retries). Allow `null` for the body so missing payloads don't 400;
  // the handler applies the documented `wait: true` default.
  //
  // Naming: the field is `wait` (not the legacy `flush`) because the
  // hyper-sdk fix made `drive.update({ wait: true })` the canonical
  // signal for "block until at least one peer has been observed". The
  // old `flush` option was silently ignored by hyperdrive v13 and caused
  // announce(true) to return before any peer was seen.
  //
