...
import { IconChevronDown, IconDownload, IconEye, IconFile, IconFolder, IconVideo, IconSortArrow } from '../../../components/icons/Icons';
...
function buildInitialExpandedState(root?: ResourceTreeNode | null): ExpandedPathState {
  return pathSetToExpandedState(
    (root?.children ?? [])
      .filter((child: ResourceTreeNode) => child.type === 'directory')
      .map((child: ResourceTreeNode) => child.path),
  );
}
...
function collectDirectoriesWithNewChildren(
  ...
  nextNodes.forEach((node) => {
    ...
    const previousChildKeys = new Set((previousNode?.children ?? []).map((child: ResourceTreeNode) => `${child.type}:${child.path}`));
    const hasNewDirectChild = (node.children ?? []).some((child: ResourceTreeNode) => !previousChildKeys.has(`${child.type}:${child.path}`));
