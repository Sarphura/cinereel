...
import { HttpExceptionFilter } from './hyper.api/filters/http-exception.filter.js'
import { RAW_BODY_KEY } from './hyper.api/middleware/raw-body.middleware.js'
import { buildOpenAPI } from './hyper.api/swagger/swagger-setup.js'
import { ensureSwaggerPatch } from './hyper.api/swagger/schema-registry.js'
import { loadOrMintSharedToken } from './hyper.infrastructure/security/shared-token.js'
import { EXIT_OK, EXIT_GENERIC, EXIT_PORT_IN_USE } from './hyper.infrastructure/exit-codes.js'
...
async function main(): Promise<void> {
  ...
  if (!isProd) {
    // Print the actual token so devs can copy it into Apifox without
    // having to `cat` the file. We only do this in dev — production
    // intentionally never writes the secret to stdout.
    // eslint-disable-next-line no-console
    console.warn(
      `[hyper-agent] dev instance at ${cfg.host}:${cfg.port} — ` +
        `X-Sidecar-Token: ${sharedToken}`,
    )
  }
