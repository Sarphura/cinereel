# Hyper Agent mirrors C# App Server's Vertical Slices shape but skips Value Objects and Domain Events

The Hyper Agent's NestJS layout is `feature/<name>/{controller,module,dto}` per feature — same shape as the .NET App Server's Vertical Slices (ADR 0020). Hyper Agent additionally mirrors the repository pattern with InMemory + concrete implementations for testability.

The Hyper Agent **does not** mirror two C# DDD primitives:
...
After C# App Server adopted Vertical Slices + Value Objects + Repository + Domain Events (ADR 0020), the question is whether Hyper Agent should mirror all of those DDD primitives. Three plausible shapes:

- **Full mirror** — Hyper Agent adopts Value Objects + Domain Events to match C# App Server exactly.
...
The Hyper Agent's `services/` directory houses cross-feature domain services:
...
- **Value Objects** — Hyper Agent uses `zod` schemas (`src/infrastructure/types/`, `src/core/common/zod/schema-registry.ts`) for input validation. Types that show up in TS signatures are POJSO (`{ driveKey: string }`, not branded types).
...
- **Aggregate boundaries / Bounded Context** — Hyper Agent is single-context.
...
The Hyper Agent is essentially a thin protocol adapter over the hyper-sdk. The adapter logic doesn't have the same complexity profile. Forcing the same DDD primitives in adds ceremony without commensurate benefit.

If V2 introduces complex business logic in Hyper Agent (e.g. content-addressed deduplication), this decision is revisited.

## Trade-off accepted

- A reviewer familiar with the C# App Server may struggle to find the equivalent primitives in Hyper Agent. The ADR is the map.
