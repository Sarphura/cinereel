...
- **Hyperdrive (Node sidecar)** — drives metadata replication
...
The .NET Application Server hosts a BT-side `BandwidthPolicy` and an anti-leech monitor. Both interfaces mirror the Hyper sidecar's structure (peerKey-keyed throttling, connection-level rejection on bad behaviour).
